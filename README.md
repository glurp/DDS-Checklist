...
Your application use 2 UDP ports : 
* base-port for to be discovered, 
* (base-port+1) for data-exchange. 

If this second port is not accessible to Admin console,  you see your participant, and nothin into it...



**B) some time ddsspy (or other participant) do not see your participant. Even several month after application strartup**

In *one host, and one value  of domaineId,* you can use 120 pariticipants (see the port-numbering rule).
But By default, a peer will explore only the 4 first ports (fist 4 participantID 0..3) of of each peer of his list.

If you use several participant, ddsspy, Admin console ... , some pusher, some subscriber, each will use one participantId.
So your application can be affected to port bigger then (base-port+2*4) ( or participandId bifgger then 3)
